...
class DiskonDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->addColumn('action', 'diskon.datatables_actions')
                         ->editColumn('diskon', function ($col) {
                                if($col['tipe'] == 'rupiah'){
                                    return "Rp. ".number_format($col['diskon']);
                                }else if($col['tipe'] == 'persen'){
                                    return $col['diskon']."%";
                                }else{
                                    return $col['diskon'];
                                }
    ...
    public function query(Diskon $model)
    {
        return $model->newQuery()
            ->join('barang', 'barang.id', '=', 'diskon.barang_id')
            ->select('diskon.*', 'barang.nama as barang_nama');
    }
